function [acc_mean,acc_sum]=sparse_classifier_l1(lambda,resultt,resultl,Label,testl,contraint,cnn)
     ...
     resultl1=resultl(:,c_d(1:mm));
      for k=1:row2
        ...
        for pp=1:length(index)
         ...
         class(pp)=length(result_index);                         % simple vote 
        end
        [w11,w22]=max(class);
        lab(k)=index(w22);
      end
